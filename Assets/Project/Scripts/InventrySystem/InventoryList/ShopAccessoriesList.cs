...
public class ShopAccessoriesList : MonoBehaviour
{
    ...
    public void PopulateList(Cloth_DataSO data)
    {
        ...
        SetOuterNavigation();
        SetInnerItemNavigation();
        SetNavigation();
    }

    void SetInnerItemNavigation()
    {
        for(int i = 0; i < allItems.Count; i++)
        {
            ...
            if (i == 0)
            {
                panelNav.selectOnDown = allItems[i + 1];
            }
            else if(i == allItems.Count - 1)
            {
                panelNav.selectOnUp = allItems[i - 1];
            }
            else
            {
                panelNav.selectOnDown = allItems[i + 1];
            ...
            panelNav.selectOnLeft = ui.AllButtons[1];
            panelNav.selectOnRight = clothUiItemsButtonData[0].selectButton;
            allItems[i].navigation = panelNav;
        }
    }

    void SetOuterNavigation()
    {
        for (int i = 0; i <= ui.AllButtons.Count - 1; i++)
        {
            ...
            else if (i != ui.AllButtons.Count - 1)
            {
                ...
                panelNav.selectOnDown = ui.AllButtons[i + 1];
                panelNav.selectOnUp = ui.AllButtons[i - 1];
                panelNav.selectOnRight = allItems[0];
                ui.AllButtons[i].navigation = panelNav;
            }
        }
    }
    ...
    public void SetNavigation()
    {
        for (int index = 0; index < clothUiItemsButtonData.Count; index++)
        {
            ...
            if(index == 0)
            {
                ...
                navigation.selectOnLeft = allItems[0];
            }
